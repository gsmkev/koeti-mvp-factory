...
pnpm --filter @koeti/saas-template db:migrate   # apply migrations
pnpm --filter @koeti/saas-template db:seed      # test user: test@test.com / admin123
pnpm --filter @koeti/saas-template dev          # http://localhost:3000
```
...
2. `pnpm --filter @koeti/saas-template db:generate`
3. `pnpm --filter @koeti/saas-template db:migrate`
4. Commit the generated migration with your change

## Verify

```bash
pnpm --filter @koeti/saas-template typecheck
pnpm --filter @koeti/saas-template test        # includes the env contract check
pnpm --filter @koeti/saas-template build
```
