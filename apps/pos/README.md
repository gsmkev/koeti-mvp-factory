...
pnpm --filter @koeti/pos db:migrate   # apply migrations
pnpm --filter @koeti/pos db:seed      # test user: test@test.com / admin123
pnpm --filter @koeti/pos dev          # http://localhost:3000
```
...
2. `pnpm --filter @koeti/pos db:generate`
3. `pnpm --filter @koeti/pos db:migrate`
4. Commit the generated migration with your change

## Verify

```bash
pnpm --filter @koeti/pos typecheck
pnpm --filter @koeti/pos test        # includes the env contract check
pnpm --filter @koeti/pos build
```
