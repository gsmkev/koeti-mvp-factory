...
pnpm --filter @koeti/gastos db:migrate   # apply migrations
pnpm --filter @koeti/gastos db:seed      # test user: test@test.com / admin123
pnpm --filter @koeti/gastos dev          # http://localhost:3000
```
...
2. `pnpm --filter @koeti/gastos db:generate`
3. `pnpm --filter @koeti/gastos db:migrate`
4. Commit the generated migration with your change

## Verify

```bash
pnpm --filter @koeti/gastos typecheck
pnpm --filter @koeti/gastos test        # includes the env contract check
pnpm --filter @koeti/gastos build
```
