// stat card — exported via @koeti/ui.
    ...
    <Card data-slot="stat-card" className={cn('py-4', className)}>
      <CardContent className="px-4">
        ...
        <div className="mt-1 flex items-end justify-between gap-2">
          ...
          {trend && trend.length > 1 && <Sparkline data={trend} className="mb-1" />}
        </div>
        {delta != null && (
          <p
            className={cn(
        ...
        {delta == null && hint && <p className="mt-1 text-xs text-muted-foreground">{hint}</p>}
      </CardContent>
    </Card>
  );
}
