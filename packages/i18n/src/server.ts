...
export function createRequestConfig(loadAppMessages: (locale: Locale) => Promise<MessagesModule>) {
  return getRequestConfig(async () => {
    const [store, head] = await Promise.all([cookies(), headers()]);
    const locale = resolveLocale(store.get(LOCALE_COOKIE)?.value, head.get('accept-language'));
